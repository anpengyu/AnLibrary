package com.apy.library.net.rxbus

import com.apy.library.net.rxbus.rxrelay.PublishRelay
import com.apy.library.net.rxbus.rxrelay.Relay
    ...
    fun <T> toObservableSticky(eventType: Class<T>): Observable<T> {
        synchronized(mStickyEventMap) {
            ...
            return if (event != null) {
                observable.mergeWith(Observable.create { eventType.cast(event)?.let { it1 ->
                    it.onNext(
                        it1
                    )
                } })
